...
com.win.confirm = function(messageStr, closeCallbackFncName, opts) {
	return new Promise(function(resolve, reject) {
		...
		if (com.util.isEmpty(closeCallbackFncName)) {
			closeCallbackFncName = function(rtn) {		//com.win.openPopup 에서 객체로 clickValue 를 넘겨줌 (ex: {clickValue:true} )
				if (!com.util.isEmpty(rtn) && !com.util.isEmpty(rtn.clickValue)) {
					resolve(rtn.clickValue);	//var aaa = com.win.confirm();   이런식으로 promise 를 통해서 return 받는경우 true/false 로 return 
				} else {
					resolve(false);
				}
			};
